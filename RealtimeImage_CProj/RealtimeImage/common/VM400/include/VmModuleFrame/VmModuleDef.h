...
typedef struct 
{
	...
	int  nFlag;           // 表示哪些值有效，按位取或值。0x00-表示所有，0x01-nInterval有效，0x02-byContinuousRun有效，0x04-byStopWhenNG有效。
	char byReserve[506];  // 保留
...
typedef struct  
{
	...
	union
	{
		PROCEDURE_CUSTOM_RUNNING_POLICY_LIST struCustom; // 当nRunMode为 PROCEDURE_RUN_POLICY_CUSTOM 时有效
	}uPolicy;
	int  nCustomIndex;   // 当nRunMode为 PROCEDURE_RUN_POLICY_CUSTOM时有效。获取时，当流程数量大于MAX_RUN_POLICY_NUM时，指定取值起始索引
