...
#define HKA_CHECK_BREAK_EX(flag,loginfo)        \
	...
	if (flag)                         \
                	{                                 \
        char szOutput[256] = { 0 };\
		sprintf_s(szOutput, sizeof(szOutput), "[#Module#][ModuleId=%d] %s",m_nModuleId,loginfo);\
...
#define HKA_CHECK_ERROR_LOG(flag, nret, funName)    \
    ...
    if (flag)                         \
        {                                 \
        char szOutput[256] = { 0 };\
		sprintf_s(szOutput, sizeof(szOutput), "[#Module#] %s,return=%d",funName,nret);\
		OutputDebugStringA(szOutput);\
		return (nret);                 \
        }                                 \
    }
...
#define  MVDSDK_CATCH  catch (int nErrCode)\
        ...
        nRet = nErrCode;\
		char szOutput[256] = { 0 };\
		sprintf_s(szOutput, sizeof(szOutput), "[#Module#][moduleId=%d] MVDSDK catch int exception! nErrCode[%d]",m_nModuleId,nRet);\
		OutputDebugStringA(szOutput);\
        }\
        ...
        {\
		OutputDebugStringA("[#Module#] MVDSDK catch other exception!");\
