...
int CVmAlgModuleBase::ProcessOutput(IN const void * const hOutput, MVDSDK_BASE_MODU_INPUT &pModuInput)
{
    ...
    if (m_bOutputMaskImage)
    {
        ...
        {
            nRet = VmModule_OutputMaskImage_Mvd(hOutput, 1, pModuInput.pImageMaskObj);
...
int CVmAlgModuleBase::Process(IN void* hInput, IN void* hOutput)
{
	...
	do
	{
		...
		if (nRet != IMVS_EC_OK)
		{
			OutputDebugStringA("[#[Module#] Base ProcessOutput return");
		}
...
int CVmAlgModuleBase::GetParam(IN const char* szParamName, OUT char* pBuff, IN int nBuffSize, OUT int* pDataLen)
{
    ...
    {
        //绘制ROI/屏蔽区情况下的图像尺寸
        *pDataLen = sizeof(m_stDrawImageSize);
        if (nBuffSize <= sizeof(m_stDrawImageSize))
        {
            return IMVS_EC_PARAM_BUF_LEN;
        }
        memcpy_s(pBuff, nBuffSize, &m_stDrawImageSize, sizeof(m_stDrawImageSize));
    }
...
int CVmAlgModuleBase::AnalyzeMultipleROIMsg(IN const char * pData, IN int nDataLen, OUT BASE_MODU_ROI_DATA* const pBaseModuROIData)
{
	...
	for (size_t index = 0; index < IMVS_MULTIPLE_ROI_COUNT_MAX; index++)
	{
		...
		case IMVS_ROI_TYPE_SECTORCALIPER:
		{
			...
			IMVS_ANNULUS  stUIAnnulus = { 0.0 };
			stUIAnnulus.center.x = stCircleCaliperBox.fCenterX;
			stUIAnnulus.center.y = stCircleCaliperBox.fCenterY;
			stUIAnnulus.inner_radius = stCircleCaliperBox.fOutterRadius - roiItem.fWidth / 2;
			stUIAnnulus.outer_radius = stCircleCaliperBox.fOutterRadius + roiItem.fWidth / 2;
			stUIAnnulus.start_angle = stCircleCaliperBox.fStartAngle;
			stUIAnnulus.end_angle = stCircleCaliperBox.fEndAngle;
...
int CVmAlgModuleBase::ResetDefaultParam()
{
    ...
    if (nRet != IMVS_EC_OK)
    {
        OutputDebugStringA("[#[Module#] Get create module cause failed, nRet=[%x].");
    }
    ...
    if (nCreateCause != CREATE_CAUSE_LOADSOL)
    {
        ...
        if (NULL == pstList)
        {
            OutputDebugStringA("[#[Module#] New param value list fail.");
...
int CVmAlgModuleBase::SetParam(IN const char* szParamName, IN const char* pData, IN int nDataLen)
{
	...
	{
		OutputDebugStringA("[#[Module#] log level from  change to");
		int nLevel = 0;
	...
	{		
		...
		for (auto tempNode : vecItem)
		{
			...
			do 
			{
				...
				{
					nRet = IMVS_EC_FILE_OPEN;
					OutputDebugStringA("[#[Module#] load xml failed");
					break;
				}
...
int CVmAlgModuleBase::AnalyzeExtraIO()
{
	...
	do
	{
		...
		if (XML_SUCCESS != docSource.Parse(m_pDynamicOutData, m_pDynamicOutDataLen))
		{
			...
			OutputDebugStringA("[#[Module#] Base load xml failed.");
			break;
		}
