...
namespace RealtimeImageControl
{
    ...
    public partial class ModelConfigDialog : Window, INotifyPropertyChanged
    {
        ...
        private void DrawBox(float fX, float fY, float fW, float fH, float fA = 0)
        {
            MyBox box = new MyBox()
            {
                ...
                Angle = fA,
                Opacity = 1,
                Color = ConvertColor(0xff, 0x00, 0xcc, 0xff),
                StrokeThickness = 1,
            };
            imageView.DrawShapes(box);

            tbCenterX.Text = box.CenterX.ToString("N2");
            tbCenterY.Text = box.CenterY.ToString("N2");
        ...
        private void ReDrawButton_Click(object sender, RoutedEventArgs e)
        {
            MyBox box = new MyBox()
            {
                CenterX = float.Parse(tbCenterX.Text),
                CenterY = float.Parse(tbCenterY.Text),
        ...
        private void Create_OnClick(object sender, RoutedEventArgs e)
        {
            try
            {
                if (ModuleParam != null)
                {
                    ...
                    pointSet.PointList = new List<IPoint>();
                    pointSet.Color = ConvertColor(0xff, 0x00, 0xff, 0x00);
                    pointSet.Opacity = 1;
                    for (int i = 0; i < points.num; i++)
                    {
                        MyPoint point = new MyPoint()
                        {
                            CenterX = points.point[i].x,
                            CenterY = points.point[i].y,
                            Color = ConvertColor(0xff, 0x00, 0xff, 0x00),
                            Opacity = 1
                        };
                ...
                try
                {
                    ...
                    if (model.ImageBuffer != null && model.ImageWidth * model.ImageHeight == model.ImageBuffer.Length)
                    {
                        ...
                        for (int i = 0; i < model.ModelPoints.Count; i++)
                        {
                            MyPoint point = new MyPoint()
                            {
                                CenterX = model.ModelPoints[i].x,
                                CenterY = model.ModelPoints[i].y,
                                Color = ConvertColor(0xff, 0x00, 0xff, 0x00),
                                Opacity = 1
                            };
                            pointSet.PointList.Add(point);
    ...
    public class MyPointSet : IPointSet
    {
        ...
        public List<IPoint> PointList { get; set; }
    }
}
